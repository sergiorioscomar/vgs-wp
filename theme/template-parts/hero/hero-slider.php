...
$slide_count = $slides_query->post_count;
$has_slides = $slide_count > 0;
?>

<!-- Background Image Container (will change dynamically) -->
...
<!-- Hero Content -->
<div class="flex-grow flex flex-col justify-center items-center text-center mt-[100px] relative z-20">
	
	<!-- Arrow Left -->
	<?php if ( $has_slides && $slide_count > 1 ) : ?>
	<div class="absolute left-6 top-1/2 -translate-y-1/2 hidden md:block">
		<button id="hero-prev" class="w-11 h-11 rounded-full bg-[#9FCE00] text-white flex items-center justify-center shadow-md hover:bg-white hover:text-[#2E3A8C] transition-colors">
			<svg class="w-[20px] h-[17px] block" viewBox="0 0 20 17" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false">
				<path d="M18.7882 7.2727H4.41554L8.81463 1.98897C9.02033 1.74149 9.11924 1.42243 9.0897 1.10198C9.06015 0.781534 8.90452 0.485946 8.65703 0.280246C8.40955 0.0745462 8.09048 -0.0244169 7.77003 0.00512801C7.44958 0.0346729 7.15399 0.190306 6.94829 0.437789L0.888987 7.70897C0.848235 7.76681 0.811735 7.82757 0.779909 7.89075C0.779909 7.95135 0.779909 7.9877 0.695113 8.0483C0.640067 8.18725 0.611382 8.33516 0.610247 8.48457C0.611382 8.63398 0.640067 8.78189 0.695113 8.92084C0.695113 8.98143 0.695113 9.01779 0.779909 9.07838C0.811735 9.14157 0.848235 9.20233 0.888987 9.26016L6.94829 16.5313C7.06216 16.6681 7.2049 16.7782 7.36621 16.8536C7.52753 16.929 7.70342 16.9679 7.8814 16.9676C8.16455 16.9682 8.43904 16.8696 8.65703 16.6889C8.77974 16.5872 8.88115 16.4622 8.95553 16.3212C9.02992 16.1802 9.07569 16.0259 9.09028 15.8672C9.10487 15.7085 9.08801 15.5484 9.04074 15.3962C8.99347 15.244 8.91657 15.1026 8.81463 14.9802L4.41554 9.69643H18.7882C19.1096 9.69643 19.4178 9.56875 19.6451 9.34149C19.8724 9.11422 20.0001 8.80597 20.0001 8.48457C20.0001 8.16316 19.8724 7.85492 19.6451 7.62765C19.4178 7.40038 19.1096 7.2727 18.7882 7.2727Z" fill="currentColor"/>
			</svg>
		</button>
	</div>
	<?php endif; ?>
	...
	<!-- Slider Container -->
	<div id="hero-slider" class="w-full max-w-4xl px-4">
		<?php
		if ( $has_slides ) :
			$slides_query->rewind_posts();
			$slide_index = 0;
			while ( $slides_query->have_posts() ) :
				$slides_query->the_post();
				$button_text = get_post_meta( get_the_ID(), '_vgs_button_text', true );
				$button_link = get_post_meta( get_the_ID(), '_vgs_button_link', true );
				
				// Defaults
				if ( empty( $button_text ) ) {
					$button_text = 'CONTACTA CON NOSOTROS';
				}
				if ( empty( $button_link ) ) {
					$button_link = '#';
				}
				
				$is_active = $slide_index === 0 ? '' : 'hidden';
				?>
				<div class="hero-slide <?php echo esc_attr( $is_active ); ?>" data-slide="<?php echo esc_attr( $slide_index ); ?>">
					<?php 
					$description = get_the_content();
					if ( ! empty( $description ) ) :
					?>
					<!-- Description -->
					<div class="text-white font-semibold text-[48px] leading-[85px] tracking-[-1px] text-center font-['Roboto'] mb-2">
						<?php echo wp_kses_post( $description ); ?>
					</div>
					<?php endif; ?>
					
					<!-- Main Title -->
					<h1 class="text-[#9FCE00] font-extrabold text-[64px] leading-[85px] tracking-[-1px] text-center uppercase font-['Roboto']">
						<?php 
						$title = get_the_title();
						$words = explode(' ', $title);
						...
						if ( ! empty( $remaining_title ) ) {
							echo esc_html( $remaining_title ) . ' ';
						}
						?>
						<span class="inline-flex flex-col items-center relative">
							<span class="relative z-10"><?php echo esc_html( $last_word ); ?></span>
							<!-- Underline image -->
							<span class="mt-1 block h-[20px] w-[240px] md:w-[270px]">
								<img src="<?php echo esc_url( get_template_directory_uri() . '/assets/images/vectorSubrayado.png' ); ?>" class="w-full h-full object-contain" alt="">
							</span>
						</span>
					</h1>
					...
					<!-- Button -->
					<div class="mt-12">
						<a href="<?php echo esc_url( $button_link ); ?>" class="inline-flex items-center justify-center w-[319px] h-[59px] bg-white border-2 border-[#9FCE00] rounded-[50px] text-[#3E509D] font-bold text-[18px] leading-[24px] tracking-[0.5px] uppercase shadow-lg hover:bg-[#9FCE00] hover:text-white hover:border-[#9FCE00] transition-colors">
			...
			// Fallback slide when no slides exist
			?>
			<div class="hero-slide">
				<h1 class="font-semibold text-[48px] leading-[85px] tracking-[-1px] text-white">
					Venta y corte a medida de<br>
					<span class="font-extrabold text-[64px] leading-[85px] tracking-[-1px] text-[#9FCE00] uppercase">
						PANEL
						<span class="inline-flex flex-col items-center relative ml-2">
							<span class="relative z-10">SÁNDWICH</span>
				...
				<!-- Button Contacta -->
				<div class="mt-12">
					<a href="#" class="inline-flex items-center justify-center w-[319px] h-[59px] bg-white border-2 border-[#9FCE00] rounded-[50px] text-[#3E509D] font-bold text-[18px] leading-[24px] tracking-[0.5px] uppercase shadow-lg hover:bg-[#9FCE00] hover:text-white hover:border-[#9FCE00] transition-colors">
	...
	<div class="absolute right-6 top-1/2 -translate-y-1/2 hidden md:block">
		<button id="hero-next" class="w-11 h-11 rounded-full bg-[#9FCE00] text-white flex items-center justify-center shadow-md hover:bg-white hover:text-[#2E3A8C] transition-colors">
			<svg class="w-[20px] h-[17px] block" viewBox="0 0 20 17" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false">
				<path d="M1.21181 7.2727H15.5845L11.1854 1.98897C10.9797 1.74149 10.8808 1.42243 10.9103 1.10198C10.9399 0.781534 11.0955 0.485946 11.343 0.280246C11.5905 0.0745462 11.9095 -0.0244169 12.23 0.00512801C12.5504 0.0346729 12.846 0.190306 13.0517 0.437789L19.111 7.70897C19.1518 7.76681 19.1883 7.82757 19.2201 7.89075C19.2201 7.95135 19.2201 7.9877 19.3049 8.0483C19.3599 8.18725 19.3886 8.33516 19.3898 8.48457C19.3886 8.63398 19.3599 8.78189 19.3049 8.92084C19.3049 8.98143 19.3049 9.01779 19.2201 9.07838C19.1883 9.14157 19.1518 9.20233 19.111 9.26016L13.0517 16.5313C12.9378 16.6681 12.7951 16.7782 12.6338 16.8536C12.4725 16.929 12.2966 16.9679 12.1186 16.9676C11.8354 16.9682 11.561 16.8696 11.343 16.6889C11.2203 16.5872 11.1188 16.4622 11.0445 16.3212C10.9701 16.1802 10.9243 16.0259 10.9097 15.8672C10.8951 15.7085 10.9119 15.5484 10.9592 15.3962C11.0065 15.244 11.0834 15.1026 11.1854 14.9802L15.5845 9.69643H1.21181C0.890398 9.69643 0.582155 9.56875 0.354887 9.34149C0.127619 9.11422 -5.91278e-05 8.80597 -5.91278e-05 8.48457C-5.91278e-05 8.16316 0.127619 7.85492 0.354887 7.62765C0.582155 7.40038 0.890398 7.2727 1.21181 7.2727Z" fill="currentColor"/>
			</svg>
		</button>
	</div>
	<?php endif; ?>
